...
import * as _Watcher from '@~crazy/watcher';
...
data.addWatch('style', {
	handler(record, bubble) {
		// 表示当前 style 的数据记录，因为子孙属性改变时 style 也改变了，此时的 record 会包含一个 bubble 属性指向发生变化的源
		console.log(record);
		// 平时 bubble 为 null， 当子孙属性发生变化时，如果祖先使用了深观察，那就会开始向上冒泡，bubble 表示真正修改的数据记录
		console.log(bubble);
	},
	deep: true,
});
 ...
 * 代理方式的好处是在代码执行时通常获取到代理对象时无法知道路径，使用代理方式可以忽略路径直接观察
 **/
...
// 保护 height 属性，保护后无法修改属性的值，只能通过函数返回的 setter 函数进行设置
...
// 尝试修改
data.style.borderWidth = 10; // 输出： build.ts:102 Assignment to readonly variable 'borderWidth'
</script>
